...
~files = "~/Documents/data/MagneticStorm12-15\ March2016_NOA\'s\ magnetometer/*.dat.txt".pathMatch;
...
(
// first load sound sample in the buffer

	~buffers = Buffer.read(s,"~/Documents/sounds/PianoPhrase01.wav");

// Create synthdef granulator

	SynthDef(\granulatorAn, {| gate = 1, freq = 1000, freq2 = 5000, freqblow = 10, rq = 0.25, modfreq = 122, ind = 0.5, amp = 0.5, bufnum, envbuf, trig = 1, dur = 0.01, rate = 1, pos = 0.3, pan = 0, vol = 0.1|
...
        source = GrainBuf.ar(2, Dust.kr(trig), dur*LFNoise1.ar(1).range(1, 3), bufnum, BufRateScale.kr(bufnum)*(modulator/440)*rate, pos, 2, LFNoise1.ar(pan).range(-1, 1), envbuf)*env;		source = LPF.ar(source, freq2);
...
row[0].abs.cos.postln,
 \rate, 1, \freq2, row[1].pow(2).postln, \vol, 1, \pan, row[0].abs.sin.postln, 
\envbuf, -1]);,


		 Synth(\klank01, [\freq, row[0].abs.tan.postln, \freq2, row[1].abs.squared.postln, \freq3, row[2].abs.squared.postln, \cutoff, 6000,  //row[1].abs.pow(2).postln,
